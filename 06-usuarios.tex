...
\begin{itemize}
    ...
    \item Proceso de alta, baja, y modificación de un usuario.
        ...
        \end{itemize}
    \item \emph{Pluggable Authentication Module (PAM)} y \emph{Lightweight
...
\begin{frame}
    ...
    \frametitle{\emph{Pluggable Authentication Module (PAM)}}

    Conjunto de bibliotecas que realizan las tareas de autenticación. Permite
    separar los mecanismos de autenticación de las aplicaciones que las
    requieren.

    \begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{img/pamDiag.pdf}
        \captionsetup{textfont=tiny,labelformat=empty,justification=centering}
        \caption{\ccPublicDomain\cite{pamDiag}}
    \end{figure}
...
\begin{frame}
    ...
    \frametitle{\emph{Lightweight Directory Access Protocol (LDAP)}}

    Es un protocolo de consulta y administración de datos sobre los objetos de
    una organización. Un uso común en los sistemas \emph{GNU/LINUX} es el de
    mantener la información de las cuentas de usuarios de manera centralizada.
